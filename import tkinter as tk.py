...
class WelcomePage(tk.Toplevel):
    def __init__(self, master, db_helper):
        ...
        self.title("Welcome to Flight Management System")
    ...
    def create_widgets(self):
        ...
        img_path = "D:/ROSHNI/SRM/2nd yr/4th srm/DBMS/project/Risa/flight img.PNG"
        ...
        background_label.place(x=0, y=0, relwidth=1, relheight=1)  # Set label size to fit the window
        
        # Create a button to navigate to the main page
        main_page_btn = tk.Button(self, text="Go to Main Page", command=self.open_main_page)
        main_page_btn.place(relx=0.5, rely=0.8, anchor="center")  # Place the button in the center of the window

    def open_main_page(self):
        self.destroy()  # Close the welcome page
        app = GUI(db_helper)  # Open the main page
        app.mainloop()
...
class GUI(tk.Tk):
    def __init__(self, db_helper):
        super().__init__()
        self.db_helper = db_helper
        self.title("Jet Airbase Dashboard")  # Change window title
        self.create_widgets()

    def create_widgets(self):
        # Customizing the background color
        self.configure(bg="black")  # Change background color to black

        # Title label with custom font and color
        tk.Label(self, text="Welcome to Jet Airways", font=("Helvetica", 24, "bold"), bg="black", fg="#333333").grid(row=0, column=0, columnspan=4, pady=(20, 10))

        # Button styles
        button_style = {"font": ("Helvetica", 14), "bg": "#007bff", "fg": "white", "width": 15, "height": 2}

        # Insert Flight button
        insert_flight_btn = tk.Button(self, text="Insert Flight", command=self.show_insert_flight, **button_style)
        insert_flight_btn.grid(row=1, column=0, pady=10, padx=20)

        # Insert Passenger button
        insert_passenger_btn = tk.Button(self, text="Insert Passenger", command=self.show_insert_passenger, **button_style)
        insert_passenger_btn.grid(row=1, column=1, pady=10, padx=20)
        ...
        view_flight_btn.grid(row=1, column=2, pady=10, padx=20)
        ...
        view_passenger_btn.grid(row=1, column=3, pady=10, padx=20)
...
if __name__ == "__main__":
    db_helper = DBHelper()
    welcome_page = WelcomePage(None, db_helper)
